...
import (
	...
	toolsPkg "github.com/svpc-ai/svpc/internal/llm/tools"
...
func newCopilotClient(opts providerClientOptions) CopilotClient {
	...
	if copilotOpts.bearerToken != "" {
		...
		if githubToken == "" {
			logging.Error("GitHub token is required for Copilot provider. Set GITHUB_TOKEN environment variable, configure it in svpc.json, or ensure GitHub CLI/Copilot is properly authenticated.")
