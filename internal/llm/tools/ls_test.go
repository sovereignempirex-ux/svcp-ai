...
func TestLsTool_Run(t *testing.T) {
	// An empty path resolves against the configured working directory, so the
	// configuration has to be loaded before the tool runs.
	...
	t.Run("handles relative path", func(t *testing.T) {
		...
		// The tool resolves a relative path against the configured working
		// directory, not the process directory, so both have to agree.
		wd := config.Get().WorkingDir
		config.Get().WorkingDir = parentDir
		defer func() { config.Get().WorkingDir = wd }()
...
func TestCreateFileTree(t *testing.T) {
	// createFileTree splits on the platform separator, so the fixture has to be
	// built the same way.
	paths := []string{
		filepath.Join("path", "to", "file1.txt"),
		filepath.Join("path", "to", "dir1", "file2.txt"),
		filepath.Join("path", "to", "dir1", "subdir", "file3.txt"),
		filepath.Join("path", "to", "dir2", "file4.txt"),
	}
...
func TestPrintTree(t *testing.T) {
	...
	// printTree joins with the platform separator, so the expectations have to
	// use it too.
