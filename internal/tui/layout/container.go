...
type container struct {
	...
	// surface renders the container on the elevated (one step lighter)
	// background instead of the base canvas, which is what gives panels
	// their "glass on top of the app" depth.
	surface bool
}
...
func (c *container) View() string {
	...
	if c.surface {
		style = style.Background(t.BackgroundSecondary())
	} else {
...
// WithSurface renders the container on the elevated glass background
// (BackgroundSecondary). Use it for panels that should read as separate
// layers: the sidebar, the command bar, dialogs.
func WithSurface() ContainerOption {
	return func(c *container) {
		c.surface = true
	}
}
