...
// metaLine renders a small trailing status line under a message (model name,
// duration, finish reason...). It deliberately paints no background so the
// line inherits whatever surface the message card sits on.
func metaLine(width int, color lipgloss.AdaptiveColor, text string) string {
...
// renderMessage draws a single chat bubble. The geometry (width + left rail)
// is owned here so every message in the transcript lines up; only the colours
// change between roles:
//
//   - user      → elevated card on the secondary surface with a violet rail,
//     so the conversation reads as a clear back and forth;
//   - assistant → stays on the canvas, and its rail only lights up in the
//     accent colour while the message is focused.
...
func renderUserMessage(msg message.Message, isFocused bool, width int, position int) uiMessage {
	...
	t := theme.CurrentTheme()
	// The user card is elevated, so attachments are drawn one step *down* (the
	// canvas colour) to read as inset tokens, matching the command bar.
	attachmentStyles := styles.Transparent().
		MarginLeft(1).
		Padding(0, 1).
	...
	if len(styledAttachments) > 0 {
		attachmentContent := styles.Transparent().Width(width).Render(lipgloss.JoinHorizontal(lipgloss.Left, styledAttachments...))
...
func renderAssistantMessage(
	...
	// Add finish info if available. Each outcome gets its own semantic colour
	// so a failed or cancelled run is scannable at a glance without reading
	// the whole transcript.
	if finished {
		model := models.SupportedModels[msg.Model].Name
		switch finishData.Reason {
		case message.FinishReasonEndTurn:
			took := formatTimestampDiff(msg.CreatedAt, finishData.Time)
			info = append(info, metaLine(width, t.TextMuted(),
				fmt.Sprintf("%s · %s", model, took)))
		case message.FinishReasonCanceled:
			info = append(info, metaLine(width, t.Warning(),
				fmt.Sprintf("%s %s · %s", styles.WarningIcon, model, "canceled")))
		case message.FinishReasonError:
			info = append(info, metaLine(width, t.Error(),
				fmt.Sprintf("%s %s · %s", styles.ErrorIcon, model, "error")))
		case message.FinishReasonPermissionDenied:
			info = append(info, metaLine(width, t.Warning(),
				fmt.Sprintf("%s %s · %s", styles.WarningIcon, model, "permission denied")))
		}
	}
	if content != "" || (finished && finishData.Reason == message.FinishReasonEndTurn) {
		...
		if isSummary {
			info = append(info, metaLine(width, t.BorderFocused(),
				fmt.Sprintf("%s %s", styles.SparkIcon, "summary")))
		}
...
func renderToolResponse(toolCall message.ToolCall, response message.ToolResult, width int) string {
	t := theme.CurrentTheme()
	plain := styles.Transparent()

	if response.IsError {
		errContent := fmt.Sprintf("%s %s", styles.ErrorIcon, strings.ReplaceAll(response.Content, "\n", " "))
	...
	case tools.GlobToolName:
		return plain.Width(width).Foreground(t.TextMuted()).Render(resultContent)
	case tools.GrepToolName:
		return plain.Width(width).Foreground(t.TextMuted()).Render(resultContent)
	case tools.LSToolName:
		return plain.Width(width).Foreground(t.TextMuted()).Render(resultContent)
...
func renderToolMessage(
	...
	// Tool blocks are deliberately quieter than messages: a hairline rail on
	// the canvas instead of a card, with the tool name carrying the accent so
	// the execution trail can be skimmed down the left edge.
	style := baseStyle.
		Width(width - 1).
		BorderLeft(true).
		BorderStyle(styles.BorderLine).
		PaddingLeft(1).
		BorderForeground(t.BorderNormal())

	response := findToolResponse(toolCall.ID, allMessages)
	toolNameText := plain.Bold(true).Foreground(t.Accent()).
