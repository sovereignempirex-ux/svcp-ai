...
import (
	...
	"github.com/svpc-ai/svpc/internal/tui/layout"
...
func (m *editorCmp) View() string {
	...
	// The prompt is the visual anchor of the command bar: an accent "❯"
	// instead of a plain greater-than sign.
	...
	if len(m.attachments) == 0 {
		return lipgloss.JoinHorizontal(lipgloss.Top, style.Render(styles.PromptIcon), m.textarea.View())
	}
	m.textarea.SetHeight(m.height - 1)
	return lipgloss.JoinVertical(lipgloss.Top,
		m.attachmentsContent(),
		lipgloss.JoinHorizontal(lipgloss.Top, style.Render(styles.PromptIcon),
			m.textarea.View()),
	)
}
...
func (m *editorCmp) attachmentsContent() string {
	...
	t := theme.CurrentTheme()
	// Chips sit on the elevated command bar, so they are drawn on the canvas
	// colour to read as inset tokens rather than flat grey blocks.
...
func CreateTextArea(existing *textarea.Model) textarea.Model {
	t := theme.CurrentTheme()
	// The composer is rendered on the elevated command bar surface, so the
	// textarea has to paint the same background to look like one element.
	...
	ta.BlurredStyle.EndOfBuffer = styles.BaseStyle().Background(bgColor).Foreground(bgColor)
	...
	ta.FocusedStyle.EndOfBuffer = styles.BaseStyle().Background(bgColor).Foreground(bgColor)

	ta.Placeholder = "Ask anything…  @ references files  ctrl+k commands"
	ta.Prompt = " "
