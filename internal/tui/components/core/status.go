...
import (
	...
	"github.com/svpc-ai/svpc/internal/tui/components/chat"
...
func formatTokensAndCost(tokens, contextWindow int64, cost float64) string {
	...
	if percentage > 80 {
		...
		formattedTokens = tokensStyle.Render(fmt.Sprintf("%.0f%%", percentage))
	}
...
func (m statusCmp) View() string {
	...
	diagnostics := m.projectDiagnostics(t)
	...
	if m.info.Msg != "" {
		...
		// Empty state: show a muted chip saying no active info
		status += styles.ChipMuted("ready")
	}
...
func (m statusCmp) projectDiagnostics(t theme.Theme) string {
	...
	if errorDiagnostics == 0 && warnDiagnostics == 0 && hintDiagnostics == 0 && infoDiagnostics == 0 {
	...
	if errorDiagnostics > 0 {
		errStr := lipgloss.NewStyle().
			Background(t.BackgroundDarker()).
			Foreground(t.Error()).
			Render(fmt.Sprintf("%s %d", styles.ErrorIcon, errorDiagnostics))
		diagnostics = append(diagnostics, errStr)
	}
	if warnDiagnostics > 0 {
		warnStr := lipgloss.NewStyle().
			Background(t.BackgroundDarker()).
			Foreground(t.Warning()).
			Render(fmt.Sprintf("%s %d", styles.WarningIcon, warnDiagnostics))
		diagnostics = append(diagnostics, warnStr)
	}
	if hintDiagnostics > 0 {
		hintStr := lipgloss.NewStyle().
			Background(t.BackgroundDarker()).
			Foreground(t.Text()).
			Render(fmt.Sprintf("%s %d", styles.HintIcon, hintDiagnostics))
		diagnostics = append(diagnostics, hintStr)
	}
	if infoDiagnostics > 0 {
		infoStr := lipgloss.NewStyle().
			Background(t.BackgroundDarker()).
			Foreground(t.Info()).
			Render(fmt.Sprintf("%s %d", styles.InfoIcon, infoDiagnostics))
...
func (m statusCmp) model() string {
	...
	return styles.Chip(model.Name, t.Secondary(), t.Background())
}
